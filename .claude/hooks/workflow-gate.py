#!/usr/bin/env python3
"""Umbrella dispatcher wrapper for the sub-project `workflow-gate.py` hook.

AGENT-OS-CLAUDE-HOOK-DISPATCHER

Issue 96. Sub-project settings run this hook as:

    cd "$CLAUDE_PROJECT_DIR" && python3 .claude/hooks/workflow-gate.py
...
try:
    ...
    sys.stderr.write(
        ...
        "workflow-gate.py; skipped without blocking\n"
    )
    raise SystemExit(0)

raise SystemExit(wrapper_main("workflow-gate.py", __file__))
