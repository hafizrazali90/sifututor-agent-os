...
const sourcePath = path.join(root, 'internal-team-sourcebook.md');
const output = path.join(root, 'internal-team-sourcebook-editions');
const source = fs.readFileSync(sourcePath, 'utf8');
if (/<script|file:\/\/|\/Users\/|fal\.env|\bRM\s*\d|\$\s*\d/i.test(source)) throw Error('Source disclosure preflight failed');
const sections = [...source.matchAll(/<!-- language: (en|ms) -->\n([\s\S]*?)(?=<!-- language: |$)/g)];
if (sections.length !== 2) throw Error('Expected exactly two language sources');
const css = fs.readFileSync(path.join(root, 'external-document.css'), 'utf8') + '\n' + fs.readFileSync(path.join(root, 'internal-sourcebook.css'), 'utf8');
...
const editions = {};
// Edition files live one directory below the master; keep local citations resolvable.
const editionMarkdown = md => md.replace(/\]\((?!https?:|#|mailto:)([^)]+)\)/g, '](' + '../$1' + ')');

for (const [, lang, rawMD] of sections) {
  const md = editionMarkdown(rawMD);
  ...
  if (chapters.length !== 23) throw Error('Expected 16 chapters and 7 appendices: ' + lang);
  const nav = [];
  const body = chapters.map((chapter, i) => {
    const heading = chapter.match(/^## (.+)$/m)[1];
    const id = i < 16 ? `i${String(i + 1).padStart(2, '0')}` : `appendix-${'abcdefg'[i - 16]}`;
    nav.push({ id, heading });
...
function documentHTML(lang, bilingual) {
  ...
  const labels = lang === 'en' ? ['Contents', 'Print / save PDF', 'Internal team draft · not for external distribution'] : ['Kandungan', 'Cetak / simpan PDF', 'Draf pasukan dalaman · bukan untuk edaran luar'];
...
function setLanguage(lang) {
  ...
  document.title=lang==='en'?'Internal team sourcebook | Kota Buku':'Buku rujukan pasukan dalaman | Kota Buku';
  ...
  document.querySelector('#draft-label').textContent=lang==='en'?'Internal team draft · not for external distribution':'Draf pasukan dalaman · bukan untuk edaran luar';
  ...
  try {localStorage.setItem('internal-team-sourcebook-language',lang)} catch(e) {}
...
let initial='${lang}';try{initial=localStorage.getItem('internal-team-sourcebook-language')||initial}catch(e){}setLanguage(initial);
...
(async () => {
  ...
  fs.writeFileSync(path.join(output, 'internal-team-sourcebook.html'), documentHTML('en', true));
  const browser = await chromium.launch({ channel: 'chrome', headless: true });
  try {
    for (const lang of ['en', 'ms']) {
      const filename = `internal-team-sourcebook-${lang}`;
      ...
      if(docxShape.tables!==expectedTables||docxShape.headings!==23)throw Error('DOCX structural check: '+JSON.stringify(docxShape));
      await page.close();
  ...
  manifest.internalCSSSHA256 = hash(path.join(root, 'internal-sourcebook.css'));
