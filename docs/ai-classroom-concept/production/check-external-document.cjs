...
const base=__dirname,dir=path.join(base,'external-kota-buku-editions');
...
(async()=>{
 ...
 assert(manifest.rendererSHA256===sha(path.join(base,'render-external-document.cjs')),'Stale renderer');
 ...
 await page.goto('file://'+path.join(dir,'kota-buku-document.html'));
 for(const [size,width,height]of [['desktop',1440,1000],['phone',390,844]]){
  ...
  for(const lang of ['en','ms']){
   ...
   assert(await page.locator('article:visible .chapter').count()===15,'Chapter count');
   assert(await page.locator('article:visible table').count()===10,'HTML tables');
   assert(await page.locator('article:visible blockquote').count()===3,'Concept cards');
   ...
   for(const chapter of ['e03','e08','e10']){
 ...
 const source=fs.readFileSync(path.join(base,'external-kota-buku-document.md'),'utf8');
 for(const [,lang,md]of source.matchAll(/<!-- language: (en|ms) -->\n([\s\S]*?)(?=<!-- language: |$)/g)){
  const html=fs.readFileSync(path.join(dir,`kota-buku-document-${lang}.html`),'utf8');
  assert(!/\/Users\/|fal\.env|SEPADU-v1|redONE|STNN|\bRM\s*\d|\$\s*\d|partner\/|CODEX-HANDOFF/i.test(html),'External disclosure scan');
  await page.goto('file://'+path.join(dir,`kota-buku-document-${lang}.html`));
  const paras=await page.locator('article p, article h1, article h2, article h3, article td, article th').allTextContents();
  const xml=execFileSync('/usr/bin/unzip',['-p',path.join(dir,`kota-buku-document-${lang}.docx`),'word/document.xml'],{encoding:'utf8'});
  const docxText=[...xml.matchAll(/<w:t(?:\s[^>]*)?>([\s\S]*?)<\/w:t>/g)].map(m=>m[1].replace(/&amp;/g,'&').replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&quot;/g,'"').replace(/&apos;/g,"'")).join('');
  const pdf=execFileSync('pdftotext',['-raw',path.join(dir,`kota-buku-document-${lang}.pdf`),'-'],{encoding:'utf8'}).replace(/Sifututor · Learnest Lab · 07\.09\.2026\s*\d+\s*\/\s*\d+/g,'');
  assert((xml.match(/<w:tbl>/g)||[]).length===10,'DOCX table count');
  assert((xml.match(/w:val="Heading1"/g)||[]).length===15,'DOCX heading count');
  ...
  const headings=marked.lexer(md).filter(t=>t.type==='heading'&&t.depth===2&&/^\d+\./.test(t.text));assert(headings.length===12,'Master chapters');
