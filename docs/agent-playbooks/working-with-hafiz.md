...
| Autonomous work packets | Hafiz can use the same natural phrases, but long-running work needs loop rules after the boundary is understood. | For "autopilot this", "keep going until fixed", or "clean up this batch", translate the phrase into finish point, loop size, stop rules, progress cadence, proof, and out-of-scope actions. |
...
| Owner-only protected changes | When a fixed safety rule requires Hafiz to perform a protected server change himself, he should not have to make repetitive manual edits in Nano. | Default to one tested copy-paste command or script that validates inputs, creates a protected backup, changes only the exact intended values atomically, preserves ownership and permissions, keeps secrets out of arguments and output, and prints only non-secret confirmation. Use an interactive editor only when automation is genuinely less safe, and explain that exception before asking Hafiz to edit. |
| Visual review presentation (corrections 2026-09-03) | UI review material must be readable without knowing any agent process vocabulary, and pictures must contain only the thing being reviewed. | Never use internal phase/round/contract jargon in chat; plain nouns only ("the job card"). Reuse ONE artifact link per review and end with exactly one question. Capture screenshots so no dev/gallery scaffolding is visible (tutor app: `sifututor://dev-gallery?review=<section-id>` renders components clean on the app background). Fill components with realistic scenario data (real subjects, RM rates, Malaysian locations, DD/MM/YYYY), verify every scenario against the real system first (entity types, API contract, real screen usage — never invented data shapes like a multi-subject request), and visually verify every capture before Hafiz sees it. |
...
| `autopilot this` / `keep going until fixed` / `clean up this batch` | autonomous work packet | What is the finish point, loop size, stop rule, progress cadence, proof needed, and out-of-scope list? |
