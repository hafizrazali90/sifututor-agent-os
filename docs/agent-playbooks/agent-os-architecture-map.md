...
| Project-by-project adoption | `sifu-tutor` and `ripple-suite` profiles are verified in umbrella docs and linked from their product repos; both still need optional baseline/readiness work before developer-staff rollout. | A shared Agent OS needs local project commands, evidence, deploy path, and critical-lane rules. |
...
2. Decide whether to apply optional baseline/readiness work for `sifu-tutor` and `ripple-suite`, or draft the next repo profile.
