...
| Multi-Agent Workflow | Decide when Codex, Claude, future LLMs, subagents, or humans should do each workflow stage. | `multi-agent-adapter-workflow.md`, parity contract, roles, switching docs, handoff, snapshot, save-session, templates. | Medium but newly added | Handoffs lose context, duplicate work, or let different agents invent different processes. | Add richer behavior fixtures for stage-to-worker recommendations after real use. |
...
| Governance | Decide how Agent OS itself changes over time. | `agent-os-governance.md`, review roadmap, mission ledger, commits, Koda, health/evals. | Medium but newly added | Rules are changed in many chats/files without versioning or review. | Use governance workflow for future Agent OS changes and add richer change-set fixtures later. |
| Evaluation Harness | Prove Agent OS behavior with repeatable checks. | `agent-os-evaluation-harness.md`, eval runner, response/state/Koda/capability/conversation fixtures, health. | Deep for local fixtures; medium for real-world traces. | Docs say one thing but hooks/agents behave differently. | Add evidence-backed repo-state fixtures, richer response scenarios, and eventually trace-based evals. |
