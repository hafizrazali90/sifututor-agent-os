...
| 2026-07-01 | Runtime Reliability needs daily operating examples, not only abstract mode names. | Hafiz approved continuing the Agent OS phase after the runtime packet. The practical gap is that future agents may understand the concept but still fail on ordinary phrases such as `what next`, `commit`, `push`, `save session`, or `proceed until done`. | Use the Daily Operating Examples in `agent-os-runtime-reliability.md` to translate common phrases into mode, loaded context, proof/state, stop point, and one recommended next action. |
| 2026-07-01 | Session Map needs a live update loop for long work. | The active map fell behind after runtime reliability packets were pushed. This proves the next weakness: the map can be useful at resume time but stale after commits, pushes, side paths, or long packets unless agents update the current pointer at state-change moments. | Use `session-map.md` Live Update Loop. Update the smallest useful map sections when the story changes: current/next snapshot, progress row, decision row, Agent Context, and Continuation Prompt. Do not rewrite history or update after every chat message. |
| 2026-07-01 | Workflow rules need a promotion path before automation. | Hafiz approved continuing after the Session Map live update loop. The next risk is making the Agent OS too heavy by turning every good rule into a hook, or too loose by leaving repeated mistakes as vague manual advice. | Use `agent-os-enforcement-drift.md` Promotion Path: manual guidance, playbook/skill, Markdown eval, local fixture, script/guard, hook reminder, hard block. Promote only when the rule is dangerous, repeated, or deterministic enough to check. |
