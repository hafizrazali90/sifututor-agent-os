...
| Claude delegation reliability | DF-001 through DF-015 (including DF-001B and DF-011B); AO-190 through AO-198, AO-204 | Launching without Claude Max readiness, leaking identity/API-key state into evidence, silent worker stalls, first-run setup waits, stale completion, dead supervision, worktree lane contention, invented usage savings, raw-output persistence, skipped independent Codex review, automatic production authority, inherited auth/gateway/provider overrides reaching a probe, a Max-looking login whose `apiProvider` is actually API-billed, or `paid_evaluation` job metadata being treated as trusted spending authorization. Checked by `scripts/agent-checks/agent-os-claude-delegation-fixture-runner.py` using a deterministic fake CLI, so health consumes no Claude tokens. |
