...
| Cross-project today briefing | AO-202; TB-001, TB-002 | Improvising dozens of repeated source calls, prompting once per Planner item, mixing reported/stale evidence with verified facts, asking approval for read-only preparation, or returning one flat list with unclear ownership. |
...
| Response shape | RS-001 through RS-012; TB-001, TB-002 | Closing work with vague "done" instead of changed, checked, state, remaining risk, next action, decision needed, practical meaning, easier explanation, formal-label translation, blocked-work clarity, and technical detail in natural language; or producing a flat today list without evidence/freshness and honest approval timing. |
...
| Active task freshness | TF-001 through TF-017; `test_agent_os_active_task_freshness.py` | Resuming a months-old `.claude/tasks/active.json` pointer because it parsed as valid JSON: a task whose steps are all finished, whose task file no longer exists, or which the project repository moved far past. Equally, the false failures that would make the signal unusable: an intentionally idle pointer treated as stale, a historical `.worktrees/` lane treated as current authority, a fresh clone's rewritten mtimes hiding real staleness, a long-running task whose pointer legitimately never changes while its task file does (TF-016), or a merged branch treated as a verdict when post-merge steps are pending (TF-014) or when the declared branch is the trunk it merged into (TF-015). Branch-merge state is therefore reported as evidence only; TF-017 pins that an abandoned merged claim is still failed by drift. Missing evidence resolves to `unprovable` and is reported, never repaired by guessing. Deterministic and offline (`--self-test`, run by health and the validation loop); the workspace scan is proven against temporary Git repositories with pinned commit dates. |
...
| Koda fixtures | KO-001 through KO-023 | Storing unsafe, vague, unscoped, duplicate, or invalid memories; trusting stale memory without current evidence; silently dropping memory work when CLI fallback is healthy; using wrong correction source; mutating Koda during bulk cleanup before a read-only audit; calling a mismatched write verified, repairing it automatically, or omitting the owner action for fields update cannot change; working around a project-scope ownership denial with a replacement memory; reading one empty compound search as proof a memory is absent; relaxing project/tag filters to obtain results. |
| Koda retrieval quality | KR-001 through KR-004; KRF-001 through KRF-004 | Important Agent OS memories can be found again by future sessions using the direct Koda helper, and a compound-query miss is attributed instead of guessed. Live cases are checked on demand by `scripts/agent-checks/agent-os-koda-retrieval-quality.py`; health checks syntax only because live retrieval depends on current Koda service state. The KRF fixtures are deterministic and offline (`--self-test`, also run by `test_koda_metadata_integrity.py`): compound miss with a narrow same-filter hit, zero everywhere, compound hit, and filter relaxation as a client defect. They prove the attribution logic, not current live Koda retrieval quality. |
| Koda write integrity | K1–K6; `test_koda_write.py`, `test_koda_write_cli.py`, `test_koda_metadata_integrity.py` | Exact-ID readback, supplied-field agreement, honest mismatch/unavailable results, duplicate handling, no blind retry and sanitized output. `test_koda_metadata_integrity.py` adds the issue #111 regressions: stored `category`/`tags` rewritten under an accepted write, the split between update-correctable and owner-only fields, tag-delta counts, project-scope ownership denial signalled inside the tool result, and an offline correction explainer that never echoes provider text. Real CLI processes run with a controlled transport; these tests prove repository wiring, not installed-client activation, live-server parity or atomic exactly-once writes. |
| Test coverage enforcement | TC-001 through TC-071 (`scripts/agent-checks/test_coverage_enforcement.py`) | Claiming a feature is covered by a test file that is absent, empty, comments only, a config, a bare directory, or weaker than the row's declared evidence; using an unreadable status, a placeholder cell, a path outside the project, or a deleted manifest to make a release gate go quiet; blocking a commit on coverage debt the change never touched. |
| Staff documentation release gate | RD-001 through RD-079 (`scripts/agent-checks/test_release_documentation.py`) | Shipping a staff-facing SIMS/Ripple change with no recorded documentation decision; claiming a change is `relevant` while naming documentation that does not exist, was written for an earlier release, or is the decision ledger itself; skipping a project's always-required artifact; deferring with `TBD`, `later`, a role, a queue, the agent itself, or the change's own issue as the follow-up; calling a change `not relevant` with no reason; deleting the ledger or a past decision to make a gate go quiet; gating a build script, dependency bump, test-only, or docs-only change merely because the repository keeps a changelog; and re-opening an unrelated historical decision during today's release. |
...
| Doc navigation | DN-001 through DN-008, NAV-01 through NAV-23 | Broken links or imports in the instruction layer, a `CLAUDE.md` that stops importing `AGENTS.md` or imports more at launch, entry files over budget, playbooks missing from the owner index, archived docs linked from the entry layer, and routes that lose their safety, critical-lane, E2E, release-documentation, improvement-loop, or handoff wiring. For each of the twelve product repos (NAV-03, NAV-04, NAV-14 to NAV-23): the project `AGENTS.md` reaches the umbrella contract and the project `CLAUDE.md` imports exactly `AGENTS.md`. Results are pass, fail, advisory (product repo still migrating; reported, not counted as passed), or unavailable (checkout absent; not evaluated, not counted). Checked by `scripts/agent-checks/agent-os-doc-navigation-check.py` and health; `--self-test` covers each state. |
...
| Claude delegation reliability | DF-001 through DF-028 (including DF-001B and DF-011B); AO-190 through AO-198, AO-204 | Launching without Claude Max readiness, leaking identity/API-key state into evidence, silent worker stalls, first-run setup waits, stale completion, dead supervision, worktree lane contention, invented usage savings, raw-output persistence, skipped independent Codex review, automatic production authority, inherited auth/gateway/provider overrides reaching a probe, a Max-looking login whose `apiProvider` is actually API-billed, `paid_evaluation` job metadata being treated as trusted spending authorization, accepting a handback that omits the explicit Builder Completion Proof fields or configured supplementary proof, satisfying proof fields by quoting the template, silently downgrading an implementation job to not-applicable, launching with an implicit workspace model selection that fails at the API before any usage is recorded, leaving a zero-token launch failure undiagnosable, mislabelling a successful worker from its own prose, losing a successful worker's complete return contract when it omits the configured handback file, recovering unsafe terminal text as that handback, recovering an error-shaped or stale terminal result that still exited 0, or letting a recovery filesystem failure destroy the job's evidence. Checked by `scripts/agent-checks/agent-os-claude-delegation-fixture-runner.py` using a deterministic fake CLI, so health consumes no Claude tokens. |
...
| Memory quality and safety | AO-041, AO-042, AO-043, AO-045, AO-046, AO-051, AO-052, AO-053 | Partly covered by Koda fixtures and the on-demand live Koda retrieval-quality runner. Live write/read behavior still needs careful non-polluting test design before it becomes routine automation. The stored-metadata rewrite and compound-query gap reported in issue #111 are reproduced deterministically offline; confirming they are fixed on the live server, and whether a real project-scope owner correction succeeds, still needs a supervised live check. |
...
| Worktree isolation routing | AO-205 | Executable routing proves that once Task Router decides isolation is required, agents use the single lifecycle `create` command instead of manually splitting worktree, lease, and dependency setup. |
| Approval continuity | AO-206 | Executable routing proves that an approved end-to-end boundary does not trigger another exact-file approval stop for an in-scope commit bundle. |
...
| Claude/Codex/Kilo behavioral parity | AO-082, AO-083, AO-084 | Structural parity and the Behavior Parity Review Standard are executable through the parity runner. Adapter readiness proves shared core, Codex, Claude, and portable Kilo wiring. The behavior trace runner checks Codex route/first move/action markers, has a deterministic Kilo event-parser/drift self-test, and can optionally ask Claude or Kilo for a structured live trace. Full response-quality comparison still needs human review because different wording is allowed. |
