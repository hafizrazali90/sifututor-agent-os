...
| Parity fixtures | Structural parity runner | Losing the shared playbook, Claude adapter, Codex adapter, Kilo native-skill path, Product Design phase mapping, Plane exception rule, or parity health wiring. |
| Behavior traces | BT-001 through BT-004 | Deterministic traces protect the expected workflow route, first move, approval boundary, and evidence language; optional live Claude and Kilo modes compare structured real-model traces without making normal health depend on live LLM output. Checked by `scripts/agent-checks/agent-os-behavior-trace-runner.py` and the validation loop. |
| Claude delegation reliability | DF-001 through DF-009; AO-190 through AO-198 | Launching without Claude Max readiness, leaking identity/API-key state into evidence, silent worker stalls, first-run setup waits, stale completion, dead supervision, worktree lane contention, invented usage savings, raw-output persistence, skipped independent Codex review, or automatic production authority. Checked by `scripts/agent-checks/agent-os-claude-delegation-fixture-runner.py` using a deterministic fake CLI, so health consumes no Claude tokens. |
| Adapter readiness | SH, CX, CL, and KO readiness checks | Claiming Codex, Claude, or Kilo is set up without proving the shared core, adapter-specific wiring, and deterministic behavior checks. The portable Kilo adapter is checked everywhere; machine-installed Kilo and live GLM traces are explicit optional evidence. Checked by `scripts/agent-checks/agent-os-adapter-readiness.py`. |
...
| Scenario Lab | RW-001 through RW-018 | Realistic Hafiz work moments across commit, push, critical lane, staff intake, agent-as-tester, memory, session map, production, forbidden secrets, validation target, readiness, product design, GitHub PR readiness, Planner read-only intake, production monitoring, stale Koda conflict, cross-adapter behavior comparison, and live evidence probe reporting. Checked by `python3 scripts/agent-checks/agent-os-scenario-lab-runner.py --target 0.90`. |
...
3. Extend live Claude/Codex/Kilo behavior comparison into a richer response-quality
