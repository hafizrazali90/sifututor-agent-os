...
| Claude delegation reliability | DF-001 through DF-019 (including DF-001B and DF-011B); AO-190 through AO-198, AO-204 | Launching without Claude Max readiness, leaking identity/API-key state into evidence, silent worker stalls, first-run setup waits, stale completion, dead supervision, worktree lane contention, invented usage savings, raw-output persistence, skipped independent Codex review, automatic production authority, inherited auth/gateway/provider overrides reaching a probe, a Max-looking login whose `apiProvider` is actually API-billed, `paid_evaluation` job metadata being treated as trusted spending authorization, accepting a handback that omits the explicit Builder Completion Proof fields or configured supplementary proof, satisfying proof fields by quoting the template, or silently downgrading an implementation job to not-applicable. Checked by `scripts/agent-checks/agent-os-claude-delegation-fixture-runner.py` using a deterministic fake CLI, so health consumes no Claude tokens. |
