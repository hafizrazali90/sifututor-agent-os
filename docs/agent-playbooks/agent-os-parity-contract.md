...
| Secret-output prevention | Use the same shared all-tool pre-use guard, short-lived metadata-only provider visual-capture boundary, hidden owner-entry path, staged artifact scan, and metadata-only failure logging. |
