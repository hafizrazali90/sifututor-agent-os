...
| AO-205 | Another session is editing this repo; start a separate implementation task | task routing / worktree isolation | Decide that isolation is required, then use `scripts/agent-checks/worktree-lifecycle.py create` so worktree creation, lease registration, dependency reuse/fallback, and setup evidence stay in one governed operation. Do not manually chain those steps. |
| AO-206 | `I already approved proceed until production is live. Why are you asking me to approve the exact files again?` | workflow correction / approval continuity | Acknowledge the repeated approval is wrong. An approved end-to-end boundary that includes commit covers the in-scope file bundle after exact inspection, guards, and reporting. Continue without another approval unless the files reveal material new scope, risk, destructive work, critical-lane expansion, or a boundary conflict. |
