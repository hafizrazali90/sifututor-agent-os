...
| AO-190 | Codex delegates heavy work to Claude without proving subscription authentication, worktree, branch, lane, or required MCP readiness | Claude delegation / preflight | Stop before launch. Run the filtered Claude Max preflight, remove `ANTHROPIC_API_KEY` from the child environment, and fix failed readiness checks without exposing identity or secrets. |
...
| AO-196 | Claude completes a job but its stream exposes no comparable usage counters | Claude delegation / measurement | Record usage as `unavailable`. Report transferred work and evidence quality separately; never estimate Codex savings from missing data. A Claude token count alone is not proof of net Codex savings: also record Codex interventions, product-code edits, review effort, correction rounds, routine status model calls, wall-clock time, Hafiz interruptions, and escaped defects against a named baseline. |
...
| AO-202 | A delegation experiment is called successful because Claude performed most of the coding | Claude delegation / net savings | Do not claim success from transferred work alone. Require the experiment record to show whether Codex took implementation back, how many interventions and correction rounds occurred, how much final review/rework remained, whether user interruptions fell, and whether the normal quality floor held. Use provider-confirmed usage only; mark non-comparable counters `unavailable`. |
| AO-203 | Codex hands Claude a large cross-module or critical build as a narrative prompt, and Claude skips hard cases or maps one weak test to several requirements | Claude delegation / sequential acceptance | Before handoff, create one sequential acceptance runbook with stable obligation IDs, a materialised per-cell ledger, fail-fix-proof checkpoints, an iterative defect loop, and a deterministic exit/set-equality check. Do not accept completion while any required primary row is missing, duplicated, failed, blocked, not run, or supported only by weaker evidence than the acceptance rule. |
| AO-204 | A 503-row final campaign passed, then a correction changed only one test fixture while all product SHAs and runtime artifacts stayed unchanged | QA / evidence freshness / workflow weight | Do not replay all 503 rows automatically, but do not run only one narrow selector either. Record and classify the exact Git delta; rerun every obligation owned or influenced by the changed file plus related regression tests for adjacent states, roles, flags, retries, recovery and cross-system contracts; preserve hashes and run identity for unaffected evidence at unchanged SHAs; and require both the normal acceptance checker and an additive freshness checker to pass. Widen to a complete replay if product/runtime code changed, impact cannot be isolated, or the frozen contract explicitly requires it. |
| AO-205 | During invoice product design, the agent finds an invoice already exists in QuickBooks but asks which option should block it from new export | product design / communication | Explain the lifecycle state first: the invoice is already in QuickBooks, so new export is not applicable. Route differing customer or invoice details to reconciliation or correction, then ask only the real remaining decision. Do not manufacture options around an action that should not exist. |
| AO-206 | Claude or Codex writes `SifuTutor` as the company/product name in human-facing prose | communication / brand integrity | Correct it to `Sifututor` (capital S, lowercase t). Preserve exact technical identifiers, repository slugs, URLs, domains, database/account names, code namespaces, third-party display names, and historical evidence; format literal identifiers as code where practical. |
