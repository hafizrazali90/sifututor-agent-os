...
def matches_live_marker(snippet: str, trace_text: str) -> bool:
    ...
    trace_text = normalize(trace_text)
    ...
    return any(
        all(token in trace_tokens for token in alternative)
...
def live_trace_errors(case: BehaviorCase, trace: dict) -> list[str]:
    ...
    first_move = normalize(str(trace.get("first_move", ""))).removeprefix("$")
    if normalized_route != case.expected_route:
        errors.append(f"route expected {case.expected_route}, observed {route or '<missing>'}")
    if normalize(case.expected_first_move) not in first_move:
        errors.append(
            f"first_move expected {case.expected_first_move}, observed {first_move or '<missing>'}"
        )
    trace_text = normalize(json.dumps(trace, sort_keys=True))
    for snippet in case.required_action_text:
...
def run_kilo_self_test() -> int:
    good_trace = {
        ...
        "memory_and_task_routing": (
            "After diagnosis confirms coding work, auto-create the GitHub issue."
        ),
    ...
    outcomes.append(
        (
            ...
            "missing live trace field: close_out"
            in live_trace_errors(CASES[0], missing_field),
        )
    )
    outcomes.append(
    ...
    outcomes.append(
        (
            "equivalent parity wording is accepted",
            matches_live_marker(
                "different wording is fine", "wording differences alone are not drift"
            ),
        )
    )
    outcomes.append(
...
def main() -> int:
    ...
    parser.add_argument(
        "--live-claude", action="store_true", help="also ask Claude CLI for live traces"
    )
    parser.add_argument(
        "--live-kilo", action="store_true", help="also ask Kilo/GLM for live traces"
    )
    parser.add_argument(
        "--require-live-kilo",
        action="store_true",
