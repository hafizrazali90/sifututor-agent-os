...
REQUIRED_FILE_PERMISSION_RULES = {
    "read": ('"*": allow', '"**/.env*": deny', '"**/live/**": deny'),
    "edit": ('"*": ask', '"**/.env*": deny', '"**/live/**": deny'),
    "write": ('"*": ask', '"**/.env*": deny', '"**/live/**": deny'),
}
...
def validate_agent(path: Path) -> str:
    ...
    for permission, required_rules in REQUIRED_FILE_PERMISSION_RULES.items():
        match = re.search(
            rf"(?ms)^  {permission}:\n(?P<body>(?:    .+\n)+)", frontmatter
        )
        if match is None:
            fail(f"{path} missing explicit {permission} permission boundary")
        missing_rules = [rule for rule in required_rules if rule not in match.group("body")]
        if missing_rules:
            fail(
                f"{path} {permission} permission missing rules: "
                + ", ".join(missing_rules)
            )
    return text
...
def validate_installed(repo_text: str, installed_agent: Path, config_path: Path) -> None:
    ...
    if agent_list.returncode != 0 or "sifututor-agent-os (primary)" not in agent_list.stdout:
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        missing_write = root / "missing-write.md"
        missing_write.write_text(repo_text.replace(write_block, ""), encoding="utf-8")
        outcomes.append(
            expect_failure(
                "missing write permission boundary is rejected",
                lambda: validate_agent(missing_write),
                "missing explicit write permission boundary",
            )
        )

        write_env_bypass = root / "write-env-bypass.md"
        write_env_bypass.write_text(
            repo_text.replace(
                write_block,
                '  write:\n    "*": ask\n    "**/live/**": deny\n',
            ),
        ...
        outcomes.append(
            expect_failure(
                "write .env bypass is rejected",
                lambda: validate_agent(write_env_bypass),
                'write permission missing rules: "**/.env*": deny',
            )
        )

        write_live_bypass = root / "write-live-bypass.md"
        write_live_bypass.write_text(
            repo_text.replace(
                write_block,
                '  write:\n    "*": ask\n    "**/.env*": deny\n',
            ),
        ...
        outcomes.append(
            expect_failure(
                "write live bypass is rejected",
                lambda: validate_agent(write_live_bypass),
                'write permission missing rules: "**/live/**": deny',
            )
        )
...
def main() -> int:
    ...
    if args.installed:
        ...
        print(
            "PASS Kilo Agent OS adapter: repo and installed adapter match; "
            "configured Z.ai/GLM default and Vision wiring are valid; runtime identity unobserved"
        )
    else:
