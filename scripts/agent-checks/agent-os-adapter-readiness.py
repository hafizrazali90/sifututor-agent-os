...
def check_kilo_adapter(
    *, check_installed: bool, require_installed: bool
) -> list[CheckResult]:
    ...
    checks = [
        ...
        (
            ...
            [
                sys.executable,
                "scripts/agent-checks/kilo-agent-os-adapter-check.py",
                "--self-test",
            ],
    ...
    if check_installed:
        ...
        results.append(
            CheckResult(
                ...
                passed=proc.returncode == 0,
                detail="installed Kilo agent, GLM-5.3, extension CLI, and Vision MCP pass",
...
def check_optional_live_kilo(require_live: bool) -> list[CheckResult]:
    ...
    if live.returncode == 0:
        try:
            payload = json.loads(live.stdout)
            kilo_results = [
                case.get("kilo") or {} for case in payload.get("cases", [])
            ]
        ...
        warnings.extend(
            (live.stderr or live.stdout or "live Kilo trace failed").splitlines()[-4:]
        )
